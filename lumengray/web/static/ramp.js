// Draggable exposure-ramp editor + numeric stop fields (two views of the same
// ramp). X = normalized position 0..1 (meaning depends on the caller — struts→core,
// or centre→edge, or one end→the other); Y = exposure value (255 white top .. 0
// black bottom). Drag points, click empty space to add, double-click to remove — or
// type exact position %/value in the fields below.
//
// createRamp(name, svgContainer, fieldsContainer, changeCb, initial) makes an
// INDEPENDENT editor and registers it under `name`; ramp(name) fetches it later
// (used by config.js). Each instance owns its own stops/interp/DOM.
...
const instances = {};
export function ramp(name) { return instances[name]; }

export function createRamp(name, svgContainer, fieldsContainer, changeCb, initial) {
  let stops = (initial && Array.isArray(initial.stops)) ? initial.stops.map((s) => [s[0], s[1]]) : [[0, 255], [1, 0]];
  let interp = initial && initial.interp === "step" ? "step" : "linear";
  const onChange = changeCb || (() => {});
  const fieldsEl = fieldsContainer;
  const uid = name + "-rampgrad";
  ...
  function render() { renderCurve(); renderFields(); }

  function renderCurve() {
    while (svg.firstChild) svg.removeChild(svg.firstChild);
    const defs = el("defs", {});
    const grad = el("linearGradient", { id: uid, x1: "0", x2: "1", y1: "0", y2: "0" });
    stops.forEach((s) => { const g = Math.round(s[1]); grad.appendChild(el("stop", { offset: s[0], "stop-color": `rgb(${g},${g},${g})` })); });
    defs.appendChild(grad);
    svg.appendChild(defs);
    svg.appendChild(el("rect", { x: pad, y: H - 8, width: W - 2 * pad, height: 4, fill: `url(#${uid})`, opacity: "0.9" }));
  ...
  function renderFields() {
    ...
    head.innerHTML = "<span>Pos&nbsp;%</span><span>Value</span><span></span>";
  ...
  const api = {
    getRamp: () => ({ stops: stops.map((s) => [s[0], s[1]]), interp }),
    setRamp: (r) => {
    ...
    setInterp: (m) => { interp = m === "step" ? "step" : "linear"; render(); onChange(); },
  };
  instances[name] = api;
  render();
  return api;
